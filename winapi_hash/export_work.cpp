...
const wchar_t *GetWC_(const char *c)
{
...
LPVOID get_api(uint64_t api_hash, LPCSTR module, uint64_t len, const uint64_t seed)
{
  ...
  do
  {
    ...
    if (mdl->base != nullptr)
    {
      if (!hash_lstrcmpiW(mdl->dllname.Buffer, GetWC_((LPCSTR)PRINT_HIDE_STR("kernel32.dll")))) //сравниваем имя библиотеки в буфере с необходимым
      {
        break;
      }
    }
